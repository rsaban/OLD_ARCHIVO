...
class main:
	...
	def btMostrarRegistrosClick(self, widget):
		...
		if self.rbCualquierDia.get_active() == True:
			...
			queryConsultaSolicitudes = "SELECT Fecha, Expdte, Caja, Solicitante, NecesitoHoy, Id FROM ExpedientesSolicitados ORDER BY cast(Expdte as unsigned) ASC, Caja ASC" 
			...
			queryConsultaSolicitudes = "SELECT Fecha, Expdte, Caja, Solicitante, NecesitoHoy,  Id FROM ExpedientesSolicitados WHERE Fecha = \'" + str(fechaSolicitud) + "' ORDER BY cast(Expdte as unsigned) ASC, Caja ASC" 
